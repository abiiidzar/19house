...
<div>
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="mt-6 border-t border-neutral-300 pt-5">
            ...
            <div class="hidden items-center justify-between gap-6 sm:flex">
                <p class="text-xs uppercase tracking-[0.1em] text-neutral-500">Showing <span class="font-semibold text-neutral-900">{{ $paginator->firstItem() ?? 0 }}–{{ $paginator->lastItem() ?? 0 }}</span> of <span class="font-semibold text-neutral-900">{{ $paginator->total() }}</span></p>
                <div class="flex items-center gap-1">
                    ...
                    @foreach ($elements as $element)
                        @if (is_string($element))
                            <span class="inline-flex h-11 min-w-11 items-center justify-center text-sm text-neutral-400">{{ $element }}</span>
                        @endif
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                    ...
                                    @else
                                        <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" class="inline-flex h-11 min-w-11 items-center justify-center border border-transparent px-3 text-sm text-neutral-600 transition-colors hover:border-neutral-400 hover:bg-white hover:text-black disabled:opacity-40" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">{{ $page }}</button>
                                    @endif
                                </span>
                            @endforeach
                        @endif
